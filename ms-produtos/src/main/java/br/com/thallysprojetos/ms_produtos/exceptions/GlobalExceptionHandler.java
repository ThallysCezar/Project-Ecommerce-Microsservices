package br.com.thallysprojetos.ms_produtos.exceptions;

import br.com.thallysprojetos.ms_produtos.exceptions.produtos.ProdutosAlreadyExistException;
import br.com.thallysprojetos.ms_produtos.exceptions.produtos.ProdutosException;
...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<ErrorMessage> handleFeignNotFoundException(FeignException.NotFound ex, HttpServletRequest request) {
        ...
        return buildErrorResponse(new ProdutosNotFoundException(message), request, HttpStatus.NOT_FOUND);
    }
    ...
    public ResponseEntity<ErrorMessage> handleFeignBadRequest(FeignException.BadRequest ex, HttpServletRequest request) {
        ...
        String message = "Requisição inválida ao buscar produto";
    ...
    public ResponseEntity<ErrorMessage> handleNotFoundException(ProdutosNotFoundException ex, HttpServletRequest request) {
        log.warn("Produto não encontrado: {}", ex.getMessage());
        return buildErrorResponse(ex, request, HttpStatus.NOT_FOUND);
    }

    @ExceptionHandler(ProdutosAlreadyExistException.class)
    public ResponseEntity<ErrorMessage> handleAlreadyExistException(ProdutosAlreadyExistException ex, HttpServletRequest request) {
        log.warn("Produto já existe: {}", ex.getMessage());
        return buildErrorResponse(ex, request, HttpStatus.CONFLICT);
    }

    @ExceptionHandler(ProdutosException.class)
    public ResponseEntity<ErrorMessage> handleProdutosException(ProdutosException ex, HttpServletRequest request) {
        log.error("Erro ao processar produto: {}", ex.getMessage(), ex);
